...
// ✅ Keep your full keymaps layout here
...
void rgb565_to_hsv(uint16_t rgb565, uint8_t *h, uint8_t *s, uint8_t *v) {
    uint8_t r, g, b;
...
void draw_layer_ring(uint16_t color) {
    ...
    qp_circle(display, 120, 120, 121, h, s, v, true);
    qp_circle(display, 120, 120, 115, 0, 0, 0, false);
    qp_flush(display);
}
...
uint32_t animate_text(uint32_t trigger, void *ctx) {
    ...
    uint8_t r, g, b, h_fg, s_fg, v_fg, h_bg, s_bg, v_bg;
    ...
    qp_drawtext_recolor(display, x, y, my_font, partial, h_fg, s_fg, v_fg, h_bg, s_bg, v_bg);
    qp_flush(display);

    if (anim_text[anim_step] != '\0') {
        anim_step++;
        anim_token = defer_exec(50, animate_text, NULL);
    }
